...
class LearningSessionManager:
    ...
    async def start_session(self, user_id: int, session_type: str = 'practice') -> LearningSession:
        ...
        try:
            ...
            session_id = await conn.fetchval("""
                ...
                VALUES ($1, $2, $3, $4, $5, 0)
                RETURNING id
            """, user_id, statement['id'], session_type, json.dumps({}), SessionStatus.ACTIVE.value)
    ...
    async def get_next_challenge(self, session: LearningSession) -> Optional[Challenge]:
        ...
        if session.current_trick_index >= 14:
            return None

        # Get the next trick to practice
        next_trick_id = session.current_trick_index + 1
    ...
    async def process_user_response(self, session: LearningSession, response: str, trick_id: int) -> Feedback:
        ...
        # Analyze response
        analysis = await self.feedback_engine.analyze_response(
            response, trick, statement['statement']
        )
        ...
        await self._store_user_response(
            session.id, session.user_id, trick_id, session.statement_id,
            response, feedback, analysis
        )

        # Update progress
        await self.progress_tracker.update_progress(
            session.user_id, trick_id, analysis.score, analysis.is_correct
        )
    ...
    async def _store_user_response(self, session_id: int, user_id: int, trick_id: int, 
                                 statement_id: int, response: str, feedback: Feedback, 
                                 analysis) -> None:
        ...
        try:
            ...
            """, session_id, user_id, trick_id, statement_id, response,
                feedback.analysis.feedback, analysis.score / 100, analysis.is_correct,
    ...
    async def update_session_progress(self, session: LearningSession, completed_trick_id: int) -> None:
        ...
        conn = await asyncpg.connect(self.database_url)
        try:
            # Update current trick index
            new_index = max(session.current_trick_index, completed_trick_id)
            
            await conn.execute("""
                UPDATE learning_sessions
                SET current_trick_index = $1
                WHERE id = $2
            """, new_index, session.id)
    ...
    async def complete_session(self, session: LearningSession) -> SessionSummary:
        ...
        try:
            ...
            await conn.execute("""
                ...
                WHERE id = $2
            """, SessionStatus.COMPLETED.value, session.id)
    ...
    async def _generate_session_recommendations(self, user_id: int, session_stats: Dict) -> List[str]:
        ...
        average_score = session_stats.get('average_score', 0)
        success_rate = (session_stats.get('correct_attempts', 0) / 
                       max(session_stats.get('total_attempts', 1), 1)) * 100
    ...
    async def abandon_session(self, session: LearningSession) -> None:
        ...
        try:
            await conn.execute("""
                ...
                SET status = $1
                WHERE id = $2
            """, SessionStatus.ABANDONED.value, session.id)
    ...
    async def get_adaptive_difficulty(self, user_id: int) -> str:
        ...
        if overall_progress.average_mastery >= 70:
            return "сложный"
        elif overall_progress.average_mastery >= 40:
            return "средний"
        else:
            return "легкий"
    ...
    async def cleanup_old_sessions(self, days_old: int = 7) -> int:
        ...
        try:
            ...
            count = await conn.fetchval("""
                ...
                RETURNING COUNT(*)
            """, SessionStatus.ABANDONED.value, SessionStatus.ACTIVE.value, cutoff_date)
