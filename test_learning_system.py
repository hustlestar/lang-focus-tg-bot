...
from lang_focus.learning import LearningDataLoader, TrickEngine, ProgressTracker, FeedbackEngine, LearningSessionManager
...
async def test_learning_system():
    ...
    try:
        ...
        analysis = await feedback_engine.analyze_response(test_response, trick_1, "Деньги не приносят счастья")
        ...
        session_manager = LearningSessionManager(config.database_url, trick_engine, feedback_engine, progress_tracker)
        ...
        if challenge:
            ...
            # Process response
            feedback = await session_manager.process_user_response(session, test_response, challenge.target_trick_id)
